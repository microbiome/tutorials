...
# using microbiomeutilities
library(microbiomeutilities)

ps.m3.rel <- microbiomeutilities::add_refseq(ps.m3.rel)
...
We notice that ASV ids by themselves are not informative in this case. 
```{r}
...
tax_f <- tax_table(ps.m3.rel) %>% 
  as.matrix() %>% 
  as.data.frame() %>% 
  dplyr::mutate(Genus.Species = ifelse(!is.na(Species), paste0(Genus, ".", Species), Species)) %>% 
  dplyr::select(-Species)
colnames(tax_f)[7] <- "Species"

tax_f <- as.matrix(tax_f)

tax_table(ps.m3.rel) <- tax_table(tax_f)

```

Now we add the best taxonomic classification available.  
```{r}
ps.m3.rel.f <- microbiomeutilities::format_to_besthit(ps.m3.rel)
